...
class MMFSVConfig(argparse.Namespace):
    GLOBAL: 'MMFSVConfig'

    header = f"MMF-SV: A fast structural variant (SV) caller for long-read sequencing data\n Version {BUILD}\n "
    example = """ Usage example A - Call SVs for a single sample:
       mmfsv --input sorted_indexed_alignments.bam --vcf output.vcf

       ... OR, with CRAM input and bgzipped+tabix indexed VCF output:
         mmfsv --input sample.cram --vcf output.vcf.gz

       ... OR, producing only a SNF file with SV candidates for later multi-sample calling:
         mmfsv --input sample1.bam --snf sample1.snf

       ... OR, simultaneously producing a single-sample VCF and SNF file for later multi-sample calling:
         mmfsv --input sample1.bam --vcf sample1.vcf.gz --snf sample1.snf
       ...
       ... OR, with additional options to specify tandem repeat annotations (for improved call accuracy), reference (for DEL sequences) and mosaic mode for detecting rare SVs:
         mmf-sv --input sample1.bam --vcf sample1.vcf.gz --tandem-repeats tandem_repeats.bed --reference genome.fa --mosaic

    Usage example B - Multi-sample calling:
       Step 1. Create .snf for each sample: mmfsv --input sample1.bam --snf sample1.snf
       Step 2. Combined calling: mmfsv --input sample1.snf sample2.snf ... sampleN.snf --vcf multisample.vcf
       ...
       Step 2. Combined calling: mmfsv --input snf_files_list.tsv --vcf multisample.vcf

    Usage example C - Determine genotypes for a set of known SVs (force calling):
       mmfsv --input sample.bam --genotype-vcf input_known_svs.vcf --vcf output_genotypes.vcf
       """
    usage = "mmfsv --input SORTED_INPUT.bam [--vcf OUTPUT.vcf] [--snf MERGEABLE_OUTPUT.snf] [--threads 4] [--mosaic]\n\n" + header + "\n\n" + example + "\n\n Use --help for full parameter/usage information\n \n"
    ...
    def add_main_args(parser):
        ...
        main_args.add_argument("--snf", metavar="OUT.snf", type=str, help="MMF-SV file (.snf) output filename to store candidates for later multi-sample calling", required=False)
        ...
        main_args.add_argument("-D", "--data_dir", metavar="IN", type=str, help="MMF-SV data directory", required=True)
        main_args.add_argument("-M", "--model", metavar="IN", type=str, help="Pre-trained model ot use", required=True)
        main_args.add_argument("-bs", "--batch_size", metavar="N", type=int, help="Number of Batch size to use (default: 16)", default=16)
        main_args.add_argument("-GPU", "--GPU_index", metavar="N", type=str, help="Index of GPU  to use (default: 0)", default="0")
    ...
    def add_mosaic_args(parser):
        ...
        mosaic_args.add_argument("--mosaic", help="Set MMF-SV run mode to detect rare, somatic and mosaic SVs", default=False, action="store_true")
    ...
    def __init__(self, *args, **kwargs):
        ...
        parser.add_argument("--version", action="version", version=f"MMF-SV, Version {BUILD}")
