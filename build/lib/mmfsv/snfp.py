...
# from mmfsv.genotyping import Genotype
from mmfsv.snf import SNFileBase
...
class PopulationVariant:
    ...
    def _calculate_frequency(genotypes: dict[int, tuple]) -> tuple[float, int, int]:
        ...
        genotyped_samples = 0
        variant_samples = 0
        gp = MMFSVConfig.GLOBAL.genotype_ploidy
    ...
    def from_svcall(cls, svcall: SVCall) -> Optional['PopulationVariant']:
        ...
        population_size = len(MMFSVConfig.GLOBAL.snf_input_info)

        if (pct_genotyped := (genotyped_samples / population_size)) < MMFSVConfig.GLOBAL.dev_population_min_gt:
...
class PopulationSNF(SNFileBase):
    ...
    def _create_header(self, config: MMFSVConfig, main_index: dict, snf_candidate_count: int) -> dict:
        """
