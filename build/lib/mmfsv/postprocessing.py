...
def coverage_build_requests(calls, config: MMFSVConfig):
    """
...
def coverage_fulfill(requests_for_coverage, lead_provider, config: MMFSVConfig):
...
def rescale_support(svcall, config: MMFSVConfig):
...
def qc_sv(svcall: SVCall, config: MMFSVConfig):
...
def qc_sv_post_annotate(svcall: SVCall, config: MMFSVConfig):
    af = svcall.get_info("VAF")
    af = af if af is not None else 0
    sv_is_mosaic = af <= config.mosaic_af_max
...
def genotype_sv(svcall: SVCall, config, phase: tuple | None = None):
    from mmfsv.genotyping import GENOTYPER_BY_TYPE, Genotyper
