...
class SNFileBase:
    ...
    def __init__(self, config: MMFSVConfig, handle, filename=None):
        self.config = config
        self.handle = handle
    ...
    def open(cls, filename: str) -> 'SNFileBase':
        obj = cls(MMFSVConfig.GLOBAL, open(filename, 'rb'), filename)
        obj.read_header()
        return obj
    ...
    def reqc(self) -> bool:
        """
        Was this file created by an old Version of MMF-SV we want to redo qc?
        """
    ...
    def write_results(self, config: MMFSVConfig, contigs: list[str]) -> int:
        """
