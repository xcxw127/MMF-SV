...
from mmfsv.config import MMFSVConfig
from mmfsv.snf import SNFile
from mmfsv.sv import SVCall
from mmfsv.vcf import VCF
...
class Result:
    """
    A generic result of a MMF-SV task executed by a worker process.
    """
...
class CombineResult(Result):
    ...
    def emit(self, vcf_out: VCF = None, **kwargs) -> int:
        ...
        from mmfsv.config import MMFSVConfig
        if (config := MMFSVConfig.GLOBAL).dev_population_snf:
...
class CombineResultTmpFile(CombineResult):
    ...
    def tmpfile_name(self) -> str:
        return os.path.join(MMFSVConfig.GLOBAL.tmp_dir, f'result-{self.run_id}-{self.task_id:04}.part.vcf')
    ...
    def tmpfile_unsorted(self) -> str:
        ...
        """
        return os.path.join(MMFSVConfig.GLOBAL.tmp_dir, f'result-{self.run_id}-{self.task_id:04}-unsorted.part.vcf')
    ...
    def store_calls(self, svcalls):
        ...
        if MMFSVConfig.GLOBAL.sort and svcalls:
...
class CombineResultTmpFilePopulationSNF(CombineResultTmpFile):
    ...
    def snf_filename(self) -> str:
        return os.path.join(MMFSVConfig.GLOBAL.tmp_dir, f'result-{self.run_id}-{self.task_id:04}.part.snf')
    ...
    def finalize(self):
        with open(self.snf_filename, 'wb') as handle:
            from mmfsv.snfp import PopulationSNF
            snfp = PopulationSNF(MMFSVConfig.GLOBAL, handle)
            c = 0
