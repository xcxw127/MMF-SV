...
class CallTask(Task):
    ...
    def execute(self, worker: 'MMFSVWorker' = None) -> CallResult:
        ...
        if not svcalls_unfiltered:
            svcalls_filtered = []
        else:
            chromosome = svcalls_unfiltered[0].contig

            valid_chroms = {str(i) for i in range(1, 23)} | {"X", "Y"}
            valid_chroms_with_chr = {"chr" + c for c in valid_chroms}

            if chromosome in valid_chroms or chromosome in valid_chroms_with_chr:

                MMF_calls = copy.deepcopy(svcalls_unfiltered)
                ...
                for call in MMF_calls:
                    current_count += 1

                    if call.svtype not in ["DEL", "INS"]:
                        svcalls_filtered.append(call)
                        continue
                    try:
                        pos_begin = call.pos
                        if call.svtype == "DEL":
                        ...
                        ins_img = ut.to_input_image_single(chromosome_sign[:, begin:end_])
                        ins_img_mid = ut.to_input_image_single(mid_sign[:, begin:end_])
                        ...
                        if pred_type == 0:
                            ...
                            call.id = "DEL"
                            call.svtype = "DEL"
                            if isinstance(call.info, dict):
                                call.info["svtype"] = "DEL"

                        elif pred_type == 2:
                            call.id = "INS"
                            call.svtype = "INS"
                            if isinstance(call.info, dict):
                                call.info["svtype"] = "INS"
                ...
                svcalls_filtered = copy.deepcopy(svcalls_unfiltered)

        svcalls = copy.deepcopy(svcalls_unfiltered)
...
class MMFSVParentWorker(MMFSVWorker):
    ...
    def __init__(self, config: Namespace, tasks: list[Task], **kwargs):  # noqa
        ...
        self._log = logging.getLogger('mmfsv.pworker')
